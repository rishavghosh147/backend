...
class winers(Resource): #done
    ...
    def post(self):
        ...
        if 'roll' in data:
            ...
            if ch:
                return response(jsonify({"error":"already exist"}),400)
            user=Winers(roll=data['roll'],event_name=data['event_name'],winers=winer)
            db.session.add(user)
            db.session.commit()
            return response(jsonify({"success":"successfully added"}),200)
        else:
            ...
            db.session.commit()
            return response(jsonify({"success":"successfully added"}),400)
