...
class veiw_profile(Resource): #done
    ...
    def post(self):
        ...
        return response(jsonify({"fname":f"{user.fname}","lname":f"{user.lname}","email":f"{user.email}","mobile":user.mobile,"roll":user.roll,"year":user.year,"stream":f"{user.stream}"}),200)
